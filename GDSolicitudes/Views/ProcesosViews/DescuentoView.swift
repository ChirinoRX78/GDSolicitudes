...
struct DescuentoView: View {
    @Environment(\.dismiss) private var dismiss
    let solicitudID: Int
    @State private var cargando = true
    @State private var errorMsg: String? = nil
    @State private var datos: [Descuento] = []
                                    ...
                                    VStack(spacing: 12) {
                                        ForEach(datos) { item in
                                            ...
                                            VStack(alignment: .leading, spacing: 6) {
                                                ...
                                                let articulo = "\(item.articulo) - \(item.articulodesc)"
                                                let descautact = if (item.descuentoautact) == 1 { "SI" } else { "NO" }
                                                let descautsol = if (item.descuentoautsol) == 1 { "SI" } else { "NO" }
                                                ...
                                                HStack {
                                                    Spacer()
                                                        .frame(width: 10)
                                                    Text("$\(item.descuentoact)")
                                                        .font(.system(size: 15, weight: .bold))
                                                        .foregroundColor(.white)
                                                        .padding()
                                                    Spacer()
                                                        .frame(width: 20)
                                                    Text("$\(item.descuentosol)")
                                                        .font(.system(size: 15, weight: .bold))
                                                        .foregroundColor(.white)
                                                        .padding()
                                                    Spacer()
                                                ...
                                                .padding(.bottom, -7)
                                                .padding(.top, -6)
                                                Text("Descuento por litro")
                                                    .font(.system(size: 15, weight: .bold))
                                                    .foregroundColor(Color.white)
                                                    .padding()
                                                    .frame(width: 300, height: 30)
                                                    .background(Color("Blue1"))
                                                Text("$\(item.descuentoxltr)")
                                                    .font(.system(size: 15, weight: .regular))
                                                    .foregroundColor(.black)
                                                    .multilineTextAlignment(.center)
                                                    .lineLimit(1)
                                                    ...
                                                    .fixedSize(horizontal: false, vertical: true)
                                                HStack {
                                                    Text("Descuento automático actual")
                                                        .font(.system(size: 15, weight: .bold))
                                                        .foregroundColor(Color.white)
                                                        .multilineTextAlignment(.center)
                                                        .lineLimit(2)
                                                    Spacer()
                                                        .frame(width: 30)
                                                    Text("Descuento automático solicitado")
                                                        .font(.system(size: 15, weight: .bold))
                                                        .foregroundColor(Color.white)
                                                        .multilineTextAlignment(.center)
                                                        .lineLimit(2)
                                                ...
                                                .padding(.top, -3)
                                                /*filaTablaDesc(titulo: "Solicitud", valor: "\(solicitudID)")
                                                filaTablaDesc(titulo: "Cliente", valor: cliente)
                                                filaTablaDesc(titulo: "Artículo", valor: articulo)
                                                Divider()
                                                filaTablaDesc(titulo: "Descuento actual", valor: "$\(item.descuentoact)", clave: true, claveColor: Color("LightGreen"))
                                                filaTablaDesc(titulo: "Descuento solicitado", valor: "$\(item.descuentoact)", clave: true)
                                                Divider()
                                                filaTablaDesc(titulo: "Descuento por litro", valor: "$\(item.descuentoxltr)")
                                                filaTablaDesc(titulo: "Descuento automático actual", valor: descautact)
                                                filaTablaDesc(titulo: "Descuento automático solicitado", valor: descautsol)
                                                Divider()
                                                filaTablaDesc(titulo: "Saldo", valor: "$\(item.saldo)")
                                                filaTablaDesc(titulo: "Saldo vencido", valor: "$\(item.saldovencido)")
                                                filaTablaDesc(titulo: "Último pago", valor: "Abonó: $\(item.ultpago) el \(fecpago)")
                                                Divider()
                                                filaTablaDesc(titulo: "Fecha caducidad", valor: feccad)
                                                filaTablaDesc(titulo: "Solicitante", valor: "\(item.solicitante)")*/
                                            }
                                            .padding(16)
    ...
    private func cargar() {
        cargando = true
        errorMsg = nil
        ClienteAPI.obtenerDescuento(solicitud: solicitudID) { result in
            cargando = false
            switch result {
