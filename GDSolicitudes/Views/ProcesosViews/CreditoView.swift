...
struct CreditoView: View {
    @Environment(\.dismiss) private var dismiss
    let solicitudID: Int
    @State private var cargando = true
    @State private var errorMsg: String? = nil
    @State private var datos: [Credito] = []
                                    ...
                                    VStack(spacing: 12) {
                                        ForEach(datos) { item in
                                            ...
                                            VStack(alignment: .leading, spacing: 6) {
                                                ...
                                                Divider()
                                                filaTablaCred(titulo: "Días actuales", valor: "\(item.diascredito)", clave: true, claveColor: Color("LightGreen"))
                                                filaTablaCred(titulo: "Días solicitados", valor: "\(item.diascreditosol)", clave: true)
                                                Divider()
                                                filaTablaCred(titulo: "Límite actual", valor: "$\(item.limcredito)", clave: true, claveColor: Color("LightGreen"))
                                                filaTablaCred(titulo: "Límite solicitado", valor: "$\(item.limcreditosol)", clave: true)
                                                Divider()
                                                filaTablaCred(titulo: "Datos extra actuales", valor: otroscamposact, clave: true, claveColor: Color("LightGreen"))
                                                filaTablaCred(titulo: "Datos extra solicitados", valor: otroscamposext, clave: true)
                                                Divider()
                                                filaTablaCred(titulo: "Saldo", valor: "$\(item.saldo)")
                                                filaTablaCred(titulo: "Saldo vencido", valor: "$\(item.saldovencido)")
                                                filaTablaCred(titulo: "Último pago", valor: "Abonó: $\(item.ultpago) el \(fecpago)")
                                                Divider()
                                                filaTablaCred(titulo: "Fecha caducidad", valor: feccad)
                                                filaTablaCred(titulo: "Solicitante", valor: "\(item.solicitante)")*/
                                            }
                                            .padding(16)
    ...
    private func cargar() {
        cargando = true
        errorMsg = nil
        ClienteAPI.obtenerCredito(solicitud: solicitudID) { result in
            cargando = false
            switch result {
