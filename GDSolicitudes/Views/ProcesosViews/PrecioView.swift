...
struct PrecioView: View {
    ...
    @State private var datos: [Precio] = []
                                    ...
                                    VStack(spacing: 12) {
                                        ForEach(datos) { item in
                                            ...
                                            VStack(alignment: .leading, spacing: 6) {
                                                ...
                                                let tipopreact = "\(item.tipoprecioact) - \(item.tipoprecioactdesc)"
                                                let tipopresol = "\(item.tipopreciosol) - \(item.tipopreciosoldesc)"
                                                Text("Solicitud")
                                                ...
                                                HStack {
                                                    ...
                                                    Spacer()
                                                        .frame(width: 30)
                                                    Text("Tipo de precio solicitado")
                                                        .font(.system(size: 15, weight: .bold))
                                                        .foregroundColor(Color.white)
                                                        .multilineTextAlignment(.center)
                                                        .lineLimit(2)
                                                ...
                                                .padding(.top, -3)
                                                /*filaTablaPre(titulo: "Solicitud", valor: "\(solicitudID)")
                                                filaTablaPre(titulo: "Cliente", valor: cliente)
                                                filaTablaPre(titulo: "Contrato", valor: contrato)
                                                Divider()
                                                filaTablaPre(titulo: "Tipo precio actual", valor: tipopreact, clave: true, claveColor: Color("LightGreen"))
                                                filaTablaPre(titulo: "Tipo precio solicitado", valor: tipopresol, clave: true)
                                                Divider()
                                                filaTablaPre(titulo: "Saldo", valor: "$\(item.saldo)")
                                                filaTablaPre(titulo: "Saldo vencido", valor: "$\(item.saldovencido)")
                                                filaTablaPre(titulo: "Último pago", valor: "Abonó: $\(item.ultpago) el \(fecpago)")
                                                Divider()
                                                filaTablaPre(titulo: "Fecha caducidad", valor: feccad)
                                                filaTablaPre(titulo: "Solicitante", valor: "\(item.solicitante)")*/
                                            }
                                            .padding(16)
    ...
    private func cargar() {
        cargando = true
        errorMsg = nil
        ClienteAPI.obtenerPrecio(solicitud: solicitudID) { result in
            cargando = false
            switch result {
