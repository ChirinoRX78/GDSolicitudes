...
struct Desbloqueo: Codable, Identifiable {
    let id = UUID()
    //Tabla principal
    let solicitud: Int
    ...
    let fechaini: FechaAPI
    let fechafin: FechaAPI
    let saldo: String
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case fechaini = "FechaInicialDesbloqueoSolicitado"
        case fechafin = "FechaFinalDesbloqueoSolicitado"
    ...
    var urlAutorizar: URL? {
        let urlString = "https://erpweb.intranetgd.com.mx/home/wf_ejecutarSP/\(emp)/\(procesoid)/\(solicitud)/\(cns)/\(uuid)/spDa_WfDocAutorizaSolicitudDesbloqueoUpd/\(autoriza)/2/-/\(datosAux)"
        return URL(string: urlString)
    }
    var urlRechazar: URL? {
        let urlString = "https://erpweb.intranetgd.com.mx/home/wf_ejecutarSP/\(emp)/\(procesoid)/\(solicitud)/\(cns)/\(uuid)/spDa_WfDocAutorizaSolicitudDesbloqueoUpd/\(autoriza)/3/"
        return URL(string: urlString)
    }
}
