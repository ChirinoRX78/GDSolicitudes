...
struct CreditoClientes: Codable, Identifiable {
    let id = UUID()
    //Tabla principal
    let solicitud: Int
    ...
    let factoract: String
    let factorsol: String
    let saldo: String
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case factoract = "FactorVentaActualDescripcion"
        case factorsol = "FactorVentaSolicitadoDescripcion"
    ...
    var urlAutorizar: URL? {
        let urlString = "https://erpweb.intranetgd.com.mx/home/wf_ejecutarSP/\(emp)/\(procesoid)/\(solicitud)/\(cns)/\(uuid)/spDa_WfDocAutorizaSolicitudPoliticaVentaUpd/\(autoriza)/2/-/\(datosAux)"
        return URL(string: urlString)
    }
    var urlRechazar: URL? {
        let urlString = "https://erpweb.intranetgd.com.mx/home/wf_ejecutarSP/\(emp)/\(procesoid)/\(solicitud)/\(cns)/\(uuid)/spDa_WfDocAutorizaSolicitudPoliticaVentaUpd/\(autoriza)/3/"
        return URL(string: urlString)
    }
}
