...
struct Credito: Codable, Identifiable {
    let id = UUID()
    //Tabla principal
    let solicitud: Int
    ...
    let diascreditosol: Int
    let limcreditosol: String
    let cuentasol: String
    let nom_cuentasol: String?
    let grupo: Int
    let grupodesc: String?
    let gruposol: Int
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case diascreditosol = "DiasCreditoSolicitado"
        case limcreditosol = "LimiteCreditoSolicitado"
        case cuentasol = "CuentaContableSolicitado"
        case nom_cuentasol = "CuentaContableSolicitadoNombreCuenta"
        case grupo = "GrupoActual"
        case grupodesc = "GrupoActualDescripcion"
        case gruposol = "GrupoSolicitado"
        case grupodescsol = "GrupoSolicitadoDescripcion"
    ...
    var urlAutorizar: URL? {
        let urlString = "https://erpweb.intranetgd.com.mx/home/wf_ejecutarSP/\(emp)/\(procesoid)/\(solicitud)/\(cns)/\(uuid)/spDa_WfDocAutorizaSolicitudCreditoUpd/\(autoriza)/2/-/\(datosAux)"
        return URL(string: urlString)
    }
    var urlRechazar: URL? {
        let urlString = "https://erpweb.intranetgd.com.mx/home/wf_ejecutarSP/\(emp)/\(procesoid)/\(solicitud)/\(cns)/\(uuid)/spDa_WfDocAutorizaSolicitudCreditoUpd/\(autoriza)/3/"
        return URL(string: urlString)
    }
}
