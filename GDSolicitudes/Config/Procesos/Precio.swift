...
struct Precio: Codable, Identifiable {
    let id = UUID()
    //Tabla principal
    let solicitud: Int
    ...
    let tipoprecioact: Int
    let tipopreciosol: Int
    let tipoprecioactdesc: String
    let tipopreciosoldesc: String
    let saldo: String
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case tipoprecioact = "TipoPrecioActual"
        case tipopreciosol = "TipoPrecioSolicitado"
        case tipoprecioactdesc = "TipoPrecioActualDescripcion"
        case tipopreciosoldesc = "TipoPrecioSolicitadoDescripcion"
    ...
    var urlAutorizar: URL? {
        let urlString = "https://erpweb.intranetgd.com.mx/home/wf_ejecutarSP/\(emp)/\(procesoid)/\(solicitud)/\(cns)/\(uuid)/spDa_WfDocAutorizaSolicitudPrecioAContratoUpd/\(autoriza)/2/-/\(datosAux)"
        return URL(string: urlString)
    }
    var urlRechazar: URL? {
        let urlString = "https://erpweb.intranetgd.com.mx/home/wf_ejecutarSP/\(emp)/\(procesoid)/\(solicitud)/\(cns)/\(uuid)/spDa_WfDocAutorizaSolicitudPrecioAContratoUpd/\(autoriza)/3/"
        return URL(string: urlString)
    }
}
