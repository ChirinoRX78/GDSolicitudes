...
struct Contrato: Codable, Identifiable {
    let id = UUID()
    //Tabla principal
    let solicitud: Int
    ...
    let tipocontact: Int
    let tipocontact_desc: String
    let tipocontsol: Int
    let tipocontsol_desc: String
    let saldo: String
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case tipocontact_desc = "TipoContratoActualDescripcion"
        case tipocontsol = "TipoContratoSolicitado"
        case tipocontsol_desc = "TipoContratoSolicitadoDescripcion"
    ...
    var urlAutorizar: URL? {
        let urlString = "https://erpweb.intranetgd.com.mx/home/wf_ejecutarSP/\(emp)/\(procesoid)/\(solicitud)/\(cns)/\(uuid)/spDa_WfDocAutorizaSolicitudTipoDeContratoUpd/\(autoriza)/2/-/\(datosAux)"
        return URL(string: urlString)
    }
    var urlRechazar: URL? {
        let urlString = "https://erpweb.intranetgd.com.mx/home/wf_ejecutarSP/\(emp)/\(procesoid)/\(solicitud)/\(cns)/\(uuid)/spDa_WfDocAutorizaSolicitudTipoDeContratoUpd/\(autoriza)/3/"
        return URL(string: urlString)
    }
}
